...
dependencies {
    ...
    // Gson is bundled with the IntelliJ Platform at runtime; declare compileOnly to keep the plugin zip small.
    ...
    intellijPlatform {
        // Target PyCharm Community Edition. Change to pycharmProfessional() for PyCharm Professional.
        pycharmCommunity(providers.gradleProperty("platformVersion"))

        // Plugin dependencies from JetBrains Marketplace (comma-separated in gradle.properties).
        plugins(providers.gradleProperty("platformPlugins").map { it.split(',').map(String::trim).filter(String::isNotEmpty) })

        // Bundled plugin dependencies (comma-separated in gradle.properties).
        bundledPlugins(providers.gradleProperty("platformBundledPlugins").map { it.split(',').map(String::trim).filter(String::isNotEmpty) })

        // Bundled module dependencies (comma-separated in gradle.properties).
...
intellijPlatform {
    ...
    publishing {
        ...
        // The pluginVersion is based on SemVer (https://semver.org) and supports pre-release labels like 1.0.0-beta.
...
// Register runIdeForUiTests task for UI testing with robot-server plugin
