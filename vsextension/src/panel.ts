...
export class ProjspecPanel {
        ...
        try {
            switch (msg.cmd) {
                ...
                case 'add':
                    ...
                    break;
                case 'addAdvanced':
                    await this.addProjectAdvanced(msg.path, msg.storageOptions);
                    break;
                case 'configure':
    ...
    private async addProjectAdvanced(pathOrPattern: string, storageOptions: string): Promise<void> {
        await this.withBusy(async () => {
...
<div id="add-advanced-overlay" class="hidden">
    <div id="add-advanced-modal" role="dialog" aria-modal="true" aria-labelledby="add-advanced-title">
        ...
        <div id="add-advanced-body">
            ...
            <input type="text" id="add-advanced-so" autocomplete="off" spellcheck="false" placeholder='e.g. {"key": "AKIA…", "secret": "…"}' />
        </div>
...
/* Advanced Add modal — reuses the same overlay/modal pattern as the Create-spec modal */
...
(function() {
    ...
    function submitAddAdvanced() {
        ...
        vscode.postMessage({ cmd: 'addAdvanced', path: p, storageOptions: so });
    }
